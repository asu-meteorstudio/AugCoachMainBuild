...
public class MyVideoPlayer : MonoBehaviour
{
    ...
    private XRGrabInteractable knobInteractable;
    ...
    private void Start()
    {
        ...
        knobPosY = knob.transform.localPosition.y;
        knobPosZ = knob.transform.localPosition.z;
        videoPlayer = GetComponent<VideoPlayer>();
        btnPause.SetActive(true);
        btnPlay.SetActive(false);
        videoPlayer.frame = (long)100;
        progressBarWidth = progressBarBG.GetComponent<MeshRenderer>().bounds.size.x;

        knobInteractable = knob.GetComponent<XRGrabInteractable>();
        knobInteractable.onActivate.AddListener(KnobOnPressDown);
        knobInteractable.onDeactivate.AddListener(KnobOnRelease);
        knobInteractable.onSelectEntered.AddListener(KnobOnDrag);
    }
    ...
    private void Update()
    {
        if (!knobIsDragging && !videoIsJumping)
        {
            if (videoPlayer.frameCount > 0)
            {
                float progress = (float)videoPlayer.frame / (float)videoPlayer.frameCount;
                progressBar.transform.localScale = new Vector3(progressBarWidth * progress, progressBar.transform.localScale.y, 0);
                knob.transform.localPosition = new Vector3(progressBar.transform.localPosition.x + (progressBarWidth * progress), knobPosY, knobPosZ);
            }
        }
    }

    public void KnobOnPressDown(XRBaseInteractor interactor)
    {
        VideoStop();
        minKnobX = progressBar.transform.localPosition.x;
        maxKnobX = minKnobX + progressBarWidth;
    ...
    public void KnobOnRelease(XRBaseInteractor interactor)
    {
        knobIsDragging = false;
        CalcKnobSimpleValue();
        VideoPlay();
    ...
    public void KnobOnDrag(XRBaseInteractor interactor)
    {
        knobIsDragging = true;
        videoIsJumping = true;
        Vector3 curPosition = interactor.transform.position;
        //knob.transform.position = new Vector3(curPosition.x, curPosition.y, curPosition.z);
        //newKnobX = knob.transform.localPosition.x;
        newKnobX = curPosition.x;
        if (newKnobX > maxKnobX) { newKnobX = maxKnobX; }
        if (newKnobX < minKnobX) { newKnobX = minKnobX; }
        knob.transform.localPosition = new Vector3(newKnobX, knobPosY, knobPosZ);
        CalcKnobSimpleValue();
        progressBar.transform.localScale = new Vector3(simpleKnobValue * progressBarWidth, progressBar.transform.localScale.y, progressBar.transform.localScale.z);
    }
